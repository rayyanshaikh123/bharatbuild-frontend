...
import { ownerQARequests, QAEngineerRequest } from "@/lib/api/owner-qa";

export default function OwnerQAEngineersPage() {
  const [organization, setOrganization] = useState<Organization | null>(null);
  const [requests, setRequests] = useState<QAEngineerRequest[]>([]);
  ...
  const fetchData = async () => {
    try {
      ...
      if (orgRes.organizations && orgRes.organizations.length > 0) {
        ...
        const reqsRes = await ownerQARequests.getRequests();
  ...
  const handleDecision = async (
    ...
    try {
      await ownerQARequests.updateStatus(requestId, status);
      fetchData();
    } catch (err) {
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground mt-1">
            Approve organization join requests from QA Engineers
          </p>
        </div>
